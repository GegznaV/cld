...
function works seamlessly with multiple input formats, making it a
versatile tool for creating compact letter displays from various
statistical packages and custom data.
...
| `PMCMR` / `PMCMRplus`  | PMCMR, PMCMRplus | [`kwAllPairsConoverTest()`](https://rdrr.io/pkg/PMCMRplus/man/kwAllPairsConoverTest.html), `dunnTest()`                                                                            |
| `data.frame` (rstatix) | rstatix          | [`games_howell_test()`](https://rpkgs.datanovia.com/rstatix/reference/games_howell_test.html), [`tukey_hsd()`](https://rpkgs.datanovia.com/rstatix/reference/tukey_hsd.html)       |
| `PostHocTest`          | DescTools        | [`ConoverTest()`](https://andrisignorell.github.io/DescTools/reference/ConoverTest.html), [`DunnettTest()`](https://andrisignorell.github.io/DescTools/reference/DunnettTest.html) |
...
The package supports results from **PMCMRplus** for non-parametric
post-hoc tests:
...
The **rstatix** package provides tidyverse-compatible statistical tests.
Since rstatix returns data frames, you need to specify the column names
for groups and p-values:
...
You can create CLDs from custom comparison results stored in data
frames:
...
The formula interface provides a convenient way to specify p-values and
comparison labels. There are two formula formats available:

### Single-variable formula (for pre-formatted comparison strings)

No hyphens are allowed in group names when using this format as they are
used as separators.
...
When your group names contain hyphens or special characters, use the
two-variable formula format which specifies groups separately:
